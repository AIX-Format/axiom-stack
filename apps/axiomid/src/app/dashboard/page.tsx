...
export default function Dashboard() {
  ...
  const handleToggleAgentState = async () => {
    ...
    setAgentActionStatus("Updating agent state...");
    try {
      const success = await activateAgent();
      if (success) {
        setLogs((prev) => [...prev, `[SYSTEM] Agent state toggled successfully.`]);
        setAgentActionStatus("Agent state updated!");
      } else {
        throw new Error("Failed to toggle agent state");
      }
    } catch (e: any) {
      setLogs((prev) => [...prev, `[SYSTEM] Error toggling agent state: ${e.message}`]);
      setAgentActionStatus("Error toggling state.");
    } finally {
      setTimeout(() => setAgentActionStatus(null), 3000);
    }
  };
                  ...
                  <div className="glass-panel rounded-2xl border border-white/5 hover:border-neon-green/20 transition-all p-5 flex flex-col gap-4 relative group">
                    ...
                    <div className="w-full bg-black/40 rounded-xl border border-white/5 overflow-hidden flex items-center justify-center p-2">
                      <svg className="w-full h-32 text-neon-green" viewBox="0 0 100 60" fill="none">
                        ...
                        <circle cx="45" cy="30" r="8" stroke="#00ff41" strokeWidth="0.5" strokeDasharray="1,3" className="animate-spin-slow" />
                      </svg>
                    </div>
                  ...
                  <div className="glass-panel rounded-2xl border border-white/5 hover:border-neon-green/20 transition-all p-5 flex flex-col gap-4 relative group">
                    ...
                    <div className="w-full bg-black/40 rounded-xl border border-white/5 overflow-hidden flex items-center justify-center p-2">
                      <svg className="w-full h-32 text-neon-green" viewBox="0 0 100 60" fill="none">
                        ...
                        <line x1="15" y1="40" x2="85" y2="40" stroke="rgba(0, 255, 65, 0.08)" strokeWidth="0.5" />
                        <g className="animate-pulse" style={{ animationDuration: '4s' }}>
                          <path d="M25 22 L25 38 M25 25 L32 21 M25 30 L31 27" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                          <path d="M38 22 L44 22 M41 22 L41 38" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                          <path d="M50 38 L54 30 L50 22 M54 30 L59 38 M54 30 L58 22" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                          <path d="M68 22 L68 38 M68 22 L75 22 L75 30 L68 30 M68 30 L76 38" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                        </g>
